...
encrypted profile is [`age-v1`](../../docs/spec/age-v1-profile.md).
...
The body is strict: `token` is the only field. The endpoint does not accept an
`Agmsg-Team-ID` or bearer credential because the code already selects and
authorizes one team. A successful exchange creates the credential and consumes
the token in one database transaction, returns `Cache-Control: no-store`, and
has this shape:
...
to revoke another device returns `403 credential-scope-violation`. Unknown IDs
return `404 credential-not-found`. A successful response is `Cache-Control:
...
| 404 | `credential-not-found` | Revocation target is not provisioned for the team | Stop and refresh the credential list |
