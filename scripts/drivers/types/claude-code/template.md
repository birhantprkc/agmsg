...
5. **Switch receive too — exclusive role mode.**
   ...
   d. Run `~/.agents/skills/__SKILL_NAME__/scripts/delivery.sh status claude-code "$(pwd)"` and read its **first line**.
      - **`mode: monitor` or `mode: both`**: invoke a fresh Monitor, regardless of whether step b or c applied:
        ...
        - persistent: true
      - **`mode: turn` or exactly `mode: off`**: leave it stopped, silently — `actas` must not start automatic delivery a project wasn't configured for, and this is the expected, deliberate case.
      - **`mode: off (unrecognized: ...)`**: leave it stopped too (same rule — do not guess a mode), but this is NOT the deliberate case above and must NOT be treated the same way. `delivery.sh` could not find a settings file for this project at the path it resolved, most often because the working directory does not match how the project was actually registered. **Tell the user explicitly** — e.g. "agmsg could not find a delivery configuration for this project at `<path from the message>` — delivery is stopped, but this may mean the project isn't registered here rather than that it was deliberately turned off. Check the path, or run `/__SKILL_NAME__ mode <choice>` to configure it explicitly." Do not report `actas` as complete without saying this — a silent stop here is indistinguishable from the deliberate case and is what let this go unnoticed before (#687).
...
3. If the session's active FROM was `<name>`, clear that state. Then:
   ...
   d. Run `~/.agents/skills/__SKILL_NAME__/scripts/delivery.sh status claude-code "$(pwd)"` and read its **first line**.
      - **`mode: monitor` or `mode: both`**: invoke a fresh Monitor with the default subscription (no `actas` name filter — receives every (team, agent) pair currently registered for this project that isn't held by another session):
        ...
        - description: `agmsg inbox stream`
        - persistent: true
      - **`mode: turn` or exactly `mode: off`**: leave it stopped, silently — the deliberate case.
      - **`mode: off (unrecognized: ...)`**: leave it stopped, but say so — same reasoning as the `actas` step this mirrors (#687). Do not report the drop as complete without mentioning it.
