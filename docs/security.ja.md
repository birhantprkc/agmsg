...
検算する意図のある読者に向けて書かれているので、すべての主張には、その背後に何が
在るかのラベルを付けてある。4つのラベルを使い、それらは互いに置き換えられない:

- **cited** —— このリポジトリの `file:line`。主張の大半がこれである。
- **assumption** —— 与件として扱っており、ここでは導出していない。現れる箇所でそ
  う名指ししてある。
- **out of scope** —— この文書が調べていない。「無い」の柔らかい言い方ではない。
- **inherited** —— `age` 自体の性質であって、このリポジトリの性質ではない。確かめ
  るには age の仕様を読むのであって、このツリーではない。

以前の版のこの段落は「すべての主張は citation を持つ」と書いていた。それ自体が引
用のない、文書についての主張であり、そして偽だった: 特性の表の2行が、目に見える何
にも乗っていなかった。レビューで指摘された。上のラベルが在るのは、それらが全部同
じ種類だと言い張るのではなく、どれがどれかを言うのが修正だからである。
