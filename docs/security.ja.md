...
検算する意図のある読者に向けて書かれている。**以下のすべての主張は file と line
を引用する**。引用のない主張は仮定として印を付けてある。引用がこの文書の言うとお
りのことを言っていない場合、他のすべての主張も、再確認するまで未検証として扱って
ほしい —— それが正しい対応であり、引用がここに在る理由である。

`integration/remote` で測定した。引用はこのブランチの head で解決する。分岐点
`452da72` 以降に変更されたファイルはこの文書だけなので、以下のすべての `file:line`
はどちらのリビジョンでも同一のコードを指す —— 手元にあるほうで検証してほしい。
...
`server/` に age の実装はなく、鍵素材もなく、復号経路もなく、recipient stanza を
構築しうるものも何もない。これは「サーバに鍵が渡されていない」より強い主張である
—— 渡されたとして、それをどうすればよいか知っているコードがここには無い。
...
| メッセージ内容の完全性 | **Provided** | age AEAD。サーバ自身の digest は*その機構ではない* —— 下記参照 |
...
| メタデータの秘匿性 | **Not provided** | 宛先と時刻は設計上サーバから見える |
